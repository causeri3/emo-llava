...
def _calc_intruder_ids(W, W_merged, top_k, threshold_epsilon):
    ...
    return intruder_idcs, U_merged, S_merged, V_merged
...
def reduce_intruder_dimension(
    ...
    for layer_name, layer in peft_model.named_modules():
        ...
        if use_lowrank:
            U_base, _S_base, _V_base = torch.svd_lowrank(W, q=top_k, niter=320)
            U_merged, S_merged, V_merged = torch.svd_lowrank(W_merged, q=top_k,  niter=320) #640
            # V_merged = V_merged.T

            cos_sim = (U_merged.T @ U_base).abs().max(dim=1).values
            intruder_idcs = torch.where(cos_sim < threshold_epsilon)[0].tolist()
            # if intruder deteced by lowrank, use full svd to calc U,S and
            # - to be rule out false positives and for mitigation calculation
            if intruder_idcs:
                intruder_idcs, U_merged, S_merged, V_merged =  _calc_intruder_ids(W, W_merged, top_k, threshold_epsilon)
        else:
            intruder_idcs, U_merged, S_merged, V_merged = _calc_intruder_ids(W, W_merged, top_k, threshold_epsilon)
        ...
        """
        TESTING
        # B_intruder_old = U_merged[:, intruder_idcs] @ torch.diag(S_merged)[intruder_idcs, :].sqrt()
        # A_intruder_old = (torch.diag(S_merged)[:, intruder_idcs]).sqrt() @ V_merged[intruder_idcs, :]

        # print("B_intruder_old:", B_intruder_old.shape)
        # print("A_intruder_old:", A_intruder_old.shape)

        # B_intruder = U_merged[:, intruder_idcs] * S_merged[intruder_idcs].sqrt()  # [m, k]
        # A_intruder = S_merged[intruder_idcs].sqrt().unsqueeze(1) *  V_merged[intruder_idcs, :]  # [k, n]
        # print("B_intruder:", B_intruder.shape)
        # print("A_intruder:", A_intruder.shape)

        # assert torch.allclose(B_intruder_old @ A_intruder_old, B_intruder @ A_intruder, atol=1e-5), \
        #     f"BA diff: {(B_intruder_old @ A_intruder_old - B_intruder @ A_intruder).abs().max()}"
"""
